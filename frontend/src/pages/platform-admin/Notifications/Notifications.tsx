...
import { notificationsService } from '../../../services/platform-admin/notificationsService.mock';
import { Notification } from '../../../services/platform-admin/mockData';
...
export const Notifications: React.FC = () => {
  ...
  const [formData, setFormData] = useState({ title: '', message: '', is_enabled: false });
  ...
  const handleToggleEnable = async (id: string) => {
    try {
      await notificationsService.toggleEnable(id);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setFormData({ title: '', message: '', is_enabled: false });
      await loadNotifications();
    } catch {
  ...
  const handleDelete = async () => {
    ...
    try {
      await notificationsService.delete(deleteConfirm.notificationId);
      ...
      >
        <Stack tokens={{ childrenGap: 16 }}>
          ...
          <Checkbox
            ...
            onChange={(_, checked) => setFormData({ ...formData, is_enabled: !!checked })}
          />
        </Stack>
        <DialogFooter>
              ...
              <Stack tokens={{ childrenGap: 4 }}>
                ...
                <div
                  style={{
                    ...
                    padding: '4px 12px',
                    borderRadius: 12,
                    backgroundColor: viewNotification.is_enabled ? '#dff6dd' : '#f3f2f1',
                    color: viewNotification.is_enabled ? '#107c10' : '#605e5c',
                    fontWeight: 600,
                    fontSize: 12,
                  }}
                >
                  {viewNotification.is_enabled ? 'Enabled' : 'Disabled'}
                </div>
              </Stack>
