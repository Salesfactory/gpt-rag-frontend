    ...
    return (
        ...
        >
            ...
            <div className={styles.category_filter_section}>
                <Text className={styles.category_label}>Category</Text>
                <div className={styles.category_buttons_container}>
                    {categories.map(category => (
                        ...
                        >
                            <div className={styles.category_button_icon}>{category.icon}</div>
                            <Text className={styles.category_button_text}>{category.label}</Text>
                        </button>
                    ))}
                </div>
            </div>
            ...
            <div className={styles.all_files_header}>
                <div className={styles.folder_header_left}>
                    <Folder size={24} color="#A0CB06" />
                    <Text className={styles.folder_title}>{getCurrentFolderName()}</Text>
                    <div className={styles.navigation_buttons}>
                        {currentPath && (
                            <button type="button" onClick={navigateBack} className={styles.back_button}>
                                Back
                            </button>
                        )}
                        {currentPath && (
                            <button type="button" onClick={navigateToRoot} className={styles.back_button}>
                                Go to Root
                            </button>
                        )}
                    </div>
                ...
                <div className={styles.header_actions_container}>
                    <button className={styles.new_folder_button} onClick={() => setShowNewFolderModal(true)}>
                    ...
                    <button
                        ...
                        title={
                            sortOrder === "newest" ? "Showing newest first (click to show oldest first)" : "Showing oldest first (click to show newest first)"
                        }
                    >
                        <Clock size={16} />
            ...
            {isLoading || isMovingFile || isRenaming ? (
                ...
                <div className={styles.file_list_wrapper}>
                        ...
                        >
                            <div className={styles.file_item_left}>
                                ...
                                <div className={styles.file_details}>
                                    ...
                                    <Text className={styles.file_metadata}>Drop files here to move them up one level</Text>
                                </div>
                            </div>
                        </div>
                    )}
                                ...
                                >
                                    {editing?.type === "folder" && editing?.id === folder.full_path ? (
                                        ...
                                        <>
                                            ...
                                            <Text className={styles.file_metadata}>Folder {dropTarget === folder.full_path ? "(Drop here)" : ""}</Text>
                                        </>
                                    )}
                                </div>
                            </div>
                    ...
                    {filteredFiles
                        .filter(file => !file.name.endsWith("/init.txt"))
                        .map(file => (
                            <div
                                key={file.name}
                                draggable
                                        ...
                                        {editing?.type === "file" && editing?.id === file.name ? (
                                            <div className={styles.inline_edit_container}>
                                                <TextField
                                                    ...
                                                    styles={{ fieldGroup: { height: 32, minWidth: 220 } }}
                                                />
                                                <span className={styles.inline_ext_badge}>{splitBaseAndExt(getDisplayName(file.name)).ext || ""}</span>
                                                <div className={styles.inline_edit_actions}>
            ...
            {/* New Folder Modal */}
            {showNewFolderModal && <NewFolderDialogModal closeDialog={() => setShowNewFolderModal(false)} onCreateFolder={handleCreateFolder} />}
