...
################################################
# ORGANIZATION USAGE UTILS
################################################


def create_organization_usage(organization_id, subscription_tier_id, total_allocated=None):
    """
    ...
    Args:
        ...
        total_allocated (int, optional): Total tokens allocated. If None, retrieved from subscription tier.
    ...
    if total_allocated is not None and (not isinstance(total_allocated, int) or total_allocated < 0):
        logging.error("[create_organization_usage] Invalid total_allocated value")
        raise ValueError("total_allocated must be a non-negative integer")
    ...
    try:
        ...
        # Use total_allocated from parameter or subscription tier
        if total_allocated is None:
            total_allocated = subscription_tier.get("totalAllocated", 100000)
            if not isinstance(total_allocated, int) or total_allocated < 0:
                logging.warning("[create_organization_usage] Invalid totalAllocated in tier, using default")
